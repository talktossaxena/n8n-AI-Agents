...
This n8n workflow is a personalized AI news aggregator. It automatically fetches the latest news on Artificial Intelligence from two different sources, summarizes them using an AI agent, and emails the results to you.
Here is a breakdown of how each part of the process works:
## 1. Data Collection (The Triggers and Requests)
* Schedule Trigger: This is the starting gun. It is currently set to run every minute, though for a news digest, you’d likely change this to once a day or once a week.
* HTTP Requests: The workflow reaches out to two different news APIs simultaneously:
* GNews API: Searches for "AI" articles in English.
   * NewsAPI: Performs a similar search, specifically looking for the most recent articles.
## 2. Data Preparation
* Edit Fields: These nodes clean up the incoming data. They extract just the list of "articles" from the messy API responses so the AI doesn't get overwhelmed by technical metadata.
* Merge Node: This combines the two separate lists of articles into one single stream of data to be processed together.
## 3. AI Analysis
* AI Agent: This is the "brain" of the operation. It is given a specific prompt to:
* Pick the 15 most relevant articles.
   * Summarize them in 1-2 sentences.
   * Include the source URLs and today's date.
* Anthropic Chat Model: The Agent uses Claude Opus (via the Anthropic node) to perform the actual reading and writing.
## 4. Delivery
* Send a Message (Gmail): Finally, the workflow takes the summary generated by the AI and sends it as a plain-text email to talktossaxena@gmail.com with the subject "Latest News."
